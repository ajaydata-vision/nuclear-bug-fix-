...
metadata:
  version: "1.22"
  source_commit: "0256fdb74eaeaf90dab1c0d88e20c96a6b6f87da"
...
| `references/dotnet-patterns.md` | Any .NET / ASP.NET Core bug: middleware pipeline (auth/CORS order), model binding & validation, DI lifetimes (scoped in singleton, IOptions), async/await deadlocks + ThreadPool starvation, `HttpClient` lifecycle (port exhaustion, DNS, timeouts), EF Core (change tracking, N+1, transactions, concurrency tokens, DbContext lifetime), System.Text.Json (case sensitivity, cycles, `JsonElement` lifetime), Configuration (`ASPNETCORE_ENVIRONMENT`, Key Vault, `IOptions` validation), Kestrel / IIS / reverse proxy (forwarded headers, body size limits, IIS env vars), concurrency primitives (SemaphoreSlim, ConcurrentDictionary, Interlocked), .NET version migration (Npgsql 7 UTC, NRT + EF migrations, .NET 8 AOT `JsonSerializerContext`, `IAsyncEnumerable` buffering) — skips Blazor/SignalR | 12 categories, 37 patterns, routing hints table, dotnet-counters/stack/trace diagnostic toolkit |
